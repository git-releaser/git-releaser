...
- If you are new to the project, please check out the [good first issue](https://github.com/git-releaser/git-releaser/labels/good%20first%20issue) label.
- If you are looking for something to work on, check out our [open issues](https://github.com/git-releaser/git-releaser/issues).
...
* Open an issue to let us know that you are interested in contributing
...
## Building
Building the binary is as simple as running `go build .` in the root of the repository. If you want to build the container image, you can run `docker build -t git-releaser -f container/Dockerfile .` in the root of the repository.

## Releasing
Releases of git-releaser are done using [Git-Releaser](https://github.com/git-releaser/git-releaser) and [GoReleaser](https://goreleaser.com/). The workflow looks like this:

* A PR is merged to the `main` branch:
    * Release please is triggered, creates or updates a new release PR
    * This is done with every merge to main, the current release PR is updated every time

* Merging the 'git-releaser' PR to `main`:
